...
int main()
{
    ...
    cin >> n;
    v_t<v_t<size_t>> letters(static_cast<size_t>('z' - 'a') + 1, v_t<size_t>());

    for (size_t i{0}; i < n; ++i)
    {
        cin >> c;
        letters[static_cast<size_t>(c - 'a')].push_back(i);
    }
    ...
    while (--m != -1)
    {
        ...
        v_t<size_t> aux_letters('z' - 'a' + 1, -1);

        str_t s;
        cin >> s;
        for (char &c : s)
        {
            size_t tmp_pos{static_cast<size_t>(c - 'a')};
            max = std::max(max, letters[tmp_pos][++aux_letters[tmp_pos]]);
        }
